...
private:
    ...
    HashFunction hasher; // already here
    
    // added myself
    unsigned int bucketSize; // number of buckets allocated
    unsigned int totalBuckets; // key/value pairs
    Node** bucketArray; // node array


    // You will no doubt need to add at least a few more private members
    // private functions
    void fillAllBuckets();
    void arrayCopy(Node** source, Node** &target, unsigned int count);
    void rehashBuckets(unsigned int previousSize, unsigned int newBucketSize);
};
